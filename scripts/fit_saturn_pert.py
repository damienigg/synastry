#!/usr/bin/env python3
"""
Fit Jupiter perturbation terms by comparing secular-element predictions
against JPL Horizons geocentric ecliptic longitudes.

Compares: no perturbation vs current Meeus vs JPL-fitted.
Only recommends the JPL-fitted version if it improves on Meeus.

Usage:
    python3 scripts/fit_jupiter_pert.py
    python3 scripts/fit_jupiter_pert.py --no-fetch
"""
...
def main():
    ...
    # Meeus perturbation accuracy
    meeus_res = []
    for T, _, _, ds, jpl_lon, sec_lon in data:
        pert = sat_pert_meeus(T)
        fitted_lon = geocentric_lon(PLANET, T, pert)
        meeus_res.append(ang_diff(jpl_lon, fitted_lon))
    meeus_rms = math.sqrt(sum(r**2 for r in meeus_res) / n)
    meeus_max = max(abs(r) for r in meeus_res)

    print(f"\nData points: {n}")
    print(f"\n{'Method':<25} {'RMS':>8} {'Max':>8}")
    print("-" * 43)
    print(f"{'No perturbation':<25} {base_rms:>8.4f} {base_max:>8.4f}")
    print(f"{'Current Meeus':<25} {meeus_rms:>8.4f} {meeus_max:>8.4f}")
    ...
    # Verdict
    print(f"\n{'='*60}")
    if fit_rms < meeus_rms * 0.9:  # at least 10% better
        print(f"VERDICT: JPL-fitted IMPROVES on Meeus ({meeus_rms:.4f}° → {fit_rms:.4f}°)")
        print(f"\nPROPOSED JS:")
        _print_js(selected)
    else:
        print(f"VERDICT: Meeus is adequate ({meeus_rms:.4f}° vs fitted {fit_rms:.4f}°). Keep current.")

    # Per-date table
    print(f"\n{'Date':<14} {'JPL':>10} {'Secular':>10} {'Meeus':>10} {'Fitted':>10} {'Meeus err':>10} {'Fit err':>10}")
    print("-" * 80)
    for i, (T, orig_res, cands, ds, jpl_lon, sec_lon) in enumerate(data):
        meeus_lon = geocentric_lon(PLANET, T, sat_pert_meeus(T))
        ...
        me = ang_diff(jpl_lon, meeus_lon)
        fe = ang_diff(jpl_lon, fit_lon)
        flag = ' *' if abs(fe) < abs(me) else ''
        print(f"{ds:<14} {jpl_lon:>10.3f} {sec_lon:>10.3f} {meeus_lon:>10.3f} {fit_lon:>10.3f} {me:>+10.3f} {fe:>+10.3f}{flag}")
...
def _print_js(terms):
    ...
    print(f"function satPert(T){{const {','.join(decls)},r=x=>x*Math.PI/180;return{''.join(parts)};}}")
