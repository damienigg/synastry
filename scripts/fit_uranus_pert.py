...
def main():
    ...
    for i, (T, orig_res, cands, ds, jpl_lon, sec_lon) in enumerate(data):
        meeus_lon = geocentric_lon(PLANET, T, ura_pert_meeus(T))
...
def _print_js(terms):
    ...
    print(f"function uraPert(T){{const {','.join(decls)},r=x=>x*Math.PI/180;return{''.join(parts)};}}")
